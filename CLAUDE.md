...
| FCast receiver | Flatpak (`org.fcast.Receiver`) | **unverified** | *(none yet — verify on box)* (launched via `flatpak run org.fcast.Receiver`; the receiver for casting from a phone, e.g. Grayjay. **Auto-started at boot and kept running in the background** — see *Always-on background apps* below. It has a *persistent* window — a "waiting for connection"/pairing-code screen — not a windowless daemon. `apps/available/fcast.sh` installs it. The tile's `match` is **left `null` until verified against a live window** per *Adding an app* step 3 — until then the tile spawns a duplicate instead of focus-or-launching the running instance, so verify it early. Likely an Electron app, so confirm on the box whether it's native-Wayland (`app_id="…"`) or Xwayland (`class="…"`)) |
