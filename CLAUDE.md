...
| Kodi | Flatpak (native Wayland) | `kodi` | `app_id="kodi"` (launched via `flatpak run --filesystem=/run/media tv.kodi.Kodi`; Kodi auto-selects its Wayland windowing backend under a Wayland session, so match by `app_id`. **Derived from Kodi's Wayland app_id, not yet confirmed against a live window** — verify per *Adding an app* step 3; if Kodi instead comes up under Xwayland it'll be `app_id=null`, match `class="Kodi"`. `apps/available/kodi.sh` grants the sandbox access to `/run/media` (`flatpak override --user --filesystem=/run/media`; the tile's `cmd` also passes `--filesystem` so a stock run works) and seeds a minimalist, local-files-first config into `~/.var/app/tv.kodi.Kodi/data/userdata/`: a `sources.xml` "Media" source pointing Videos/Music/Pictures/Files at `/run/media`, and a `skin.estuary` `settings.xml` that hides the non-local home-menu items — Kodi has no single "default path", so a per-section source is the idiomatic entry point) |
