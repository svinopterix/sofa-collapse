...
These two have **persistent windows** (Spotify's player; FCast's pairing-code screen), so they don't need any special hiding — a backgrounded app sitting behind the fullscreen launcher is exactly the resting state the Home button already produces. The only real problem is a **boot-time foreground race**: each maps a window at boot that `for_window [app_id=".+"] fullscreen enable` pulls to the front, and whichever maps *last* would cover Home. `start-background.sh` solves this by launching both apps, then **waiting until the window count stops growing (the kiosk + both apps have mapped) before raising the launcher last** via `go-home.sh`. The wait counts Sway-tree nodes with a `pid` (selector-agnostic, so it doesn't depend on FCast's still-unverified `app_id`), is gated on the launcher window existing, and caps at ~20s so a stuck app can't hang the box off Home. It must run under Sway to inherit `WAYLAND_DISPLAY`/`SWAYSOCK`/`XDG_RUNTIME_DIR` + the audio env (same gotcha as the bridge). **No respawn** — a crashed background app is just relaunched from its tile.

A persistent window also means a cast starting on the FCast receiver does *not* map a new window (the existing window just starts playing), so any future "bring FCast to front + pause other audio on cast-start" feature must hook the **receiver's own Play/Stop events**, not a Sway `window::new` event.
...
`vm/provision.sh` copies `index.html`, `apps.json`, `server.py` into `~/launcher/`, generates `start-kiosk.sh`, `go-home.sh`, `media-seek.sh`, `start-background.sh`, and `~/.config/sway/config`, and adds a tty1 `exec sway` hook to `~/.bash_profile`. Note that `apps.json` is **copied** at deploy time, so editing the repo copy does not affect a running install — edit `~/launcher/apps.json` (or re-run `vm/provision.sh`).

## Conventions / gotchas

- The repo copy under `src/launcher/` is the source; the deployed copy lives in `~/launcher/`. They diverge after provisioning.
- The Sway config and the `start-kiosk.sh` / `go-home.sh` / `media-seek.sh` / `start-background.sh` helpers are **generated** by `vm/provision.sh` (the source of truth). To change window rules, edit `vm/provision.sh` and re-run it, or edit `~/.config/sway/config` live and `swaymsg reload`. Note `for_window` rules apply only to windows mapped *after* the reload.
